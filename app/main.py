...
class PanelList(Widget):
    ...
    def __init__(self, content: list, style: Style = "default on default", name: str | None = None) -> None:
        super().__init__(name)
    ...
    def render(self) -> Panel:
        return Panel(self.string, style = self.style)
...
class Mono(App):
    ...
    async def on_mount(self, event) -> None:
        ...
        self.footer = Footer()
        self.shows = PanelList([entry['media']['title']['romaji']+'\n' for entry in get_user_list("CURRENT")['entries']], style="#FDB750 on default")
        await self.view.dock(self.header, edge="top")
        await self.view.dock(self.footer, edge="bottom")
        await self.view.dock(self.shows, edge="left", size=40)
